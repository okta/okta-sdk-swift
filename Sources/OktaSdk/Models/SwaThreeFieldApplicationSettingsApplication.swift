...
public final class SwaThreeFieldApplicationSettingsApplication: Codable, Hashable {
    ...
    public static func == (lhs: SwaThreeFieldApplicationSettingsApplication, rhs: SwaThreeFieldApplicationSettingsApplication) -> Bool {
        lhs.buttonSelector == rhs.buttonSelector &&
        lhs.extraFieldSelector == rhs.extraFieldSelector &&
        lhs.extraFieldValue == rhs.extraFieldValue &&
        lhs.loginUrlRegex == rhs.loginUrlRegex &&
        lhs.passwordSelector == rhs.passwordSelector &&
        lhs.targetURL == rhs.targetURL &&
        lhs.userNameSelector == rhs.userNameSelector
        
    }

    public func hash(into hasher: inout Hasher) {
        hasher.combine(buttonSelector?.hashValue)
        hasher.combine(extraFieldSelector?.hashValue)
        hasher.combine(extraFieldValue?.hashValue)
        hasher.combine(loginUrlRegex?.hashValue)
        hasher.combine(passwordSelector?.hashValue)
        hasher.combine(targetURL?.hashValue)
        hasher.combine(userNameSelector?.hashValue)
