...
public final class LogEvent: Codable, Hashable {
    ...
    public static func == (lhs: LogEvent, rhs: LogEvent) -> Bool {
        lhs.actor == rhs.actor &&
        lhs.authenticationContext == rhs.authenticationContext &&
        lhs.client == rhs.client &&
        lhs.debugContext == rhs.debugContext &&
        lhs.displayMessage == rhs.displayMessage &&
        lhs.eventType == rhs.eventType &&
        lhs.legacyEventType == rhs.legacyEventType &&
        lhs.outcome == rhs.outcome &&
        lhs.published == rhs.published &&
        lhs.request == rhs.request &&
        lhs.securityContext == rhs.securityContext &&
    ...
    public func hash(into hasher: inout Hasher) {
        ...
        hasher.combine(authenticationContext?.hashValue)
        hasher.combine(client?.hashValue)
        hasher.combine(debugContext?.hashValue)
        hasher.combine(displayMessage?.hashValue)
        hasher.combine(eventType?.hashValue)
        hasher.combine(legacyEventType?.hashValue)
        hasher.combine(outcome?.hashValue)
        hasher.combine(published?.hashValue)
        hasher.combine(request?.hashValue)
        hasher.combine(securityContext?.hashValue)
        hasher.combine(severity?.hashValue)
        hasher.combine(target?.hashValue)
        hasher.combine(transaction?.hashValue)
